...
import imagemFaltando from "../../assets/img/wp11850536.jpg";
                    ...
                    <table>
                        ...
                        <thead>
                            ...
                            <tr className="table_cabecalho">
                                {/* th => table head */}
                                <th style={{ display: props.visibilidade }}>Cartaz</th>
                                <th>Nome</th>
                                ...
                                props.lista.map((item) => (
                                    <tr className="item_lista" key={props.tipoLista === "genero" ? item.idGenero : item.IdFilme}>
                                        ...
                                        <td data-cell="Imagem"style={{ display: props.visibilidade }}>
                                            ...
                                            {/* titulo == filme */}
                                            <img className="img_cartaz" src={(item.imagem == "" || item.imagem == null || item.imagem == undefined) ? imagemFaltando : `https://localhost:7221/imagens/${item.imagem}`} alt={item.titulo} />
                                        </td>
                                        <td data-cell="Nome">
                                            {/* Primeira célula da linha: mostra o nome (se for gênero) ou título (se for filme) */}
                                            {/* titulo == filme */}
                                        ...
                                        <td data-cell="Editar">
                                            <button className="icon" onClick={() => (props.funcEditar(item))}>
                                                <img className="icon_imagem" src={Editar} alt="Caneta" />
                                            </button>
                                        </td>
                                        <td data-cell="Excluir">
                                            <button className="icon" onClick={() => props.funcExcluir(item)}>
                                                <img className="icon_imagem" src={Excluir} alt="Lixeira" />
                                            </button>
                                        </td>
                                    </tr>
                                )) 
